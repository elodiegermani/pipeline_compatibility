...
def postprocessing(data_dir, output_dir):
    ...
    if not os.path.exists(op.join(output_dir, f'postprocessed', 'mask.nii.gz')):
		...
		pipeline_list = ['fsl-5-0-0', 'fsl-5-0-1', 
    ...
    for idx, img in enumerate(img_list):
        print('Image', img)
		img_pipeline = [p for p in pipeline if p in img][0]
		...
		if not os.path.isdir(op.join(output_dir, 'postprocessed', img_pipeline, 'node-L1')):
